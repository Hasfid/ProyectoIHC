/**
 * locales/en.ts — English translations.
 * 
 * Contains all UI text in English.
 */
export const en = {
  profile: {
    title: "Profile",
    followers: "Followers",
    following: "Following",
    records: "Records",
    editProfile: "Edit Profile",
    shareProfile: "Share Profile",
    tabRecords: "My Records",
    tabCommunity: "Posts",
    noRecords: "No records in this category",
    noPosts: "No community posts",
    noPending: "No pending records",
    settings: "Settings",
    changeToEn: "Switch to English",
    changeToEs: "Switch to Spanish",
    logout: "Log Out",
    close: "Close",
    menu: "Menu",
    published: "Published",
    pending: "Pending",
    searchRecords: "Search my records...",
    changeToDark: "Switch to dark mode",
    changeToLight: "Switch to light mode",
    changePhoto: "Change photo",
    usernameLabel: "Username",
    descriptionLabel: "Description",
    descriptionPlaceholder: "Tell us about yourself...",
    usernameEmpty: "Username cannot be empty",
    usernameTaken: "That username is already taken",
    saveError: "Could not update profile: ",
    editDescription: "Edit Description",
    descriptionWritePlaceholder: "Write a description...",
    editNote: "Edit note",
    deleteRecord: "Delete record",
    deleteRecordConfirm: "Are you sure you want to delete this record permanently?",
    deleteRecordRLS: "The record was not deleted from the database. This usually happens due to missing permissions (RLS) or because the record no longer exists.",
    deleteRecordSuccess: "The record has been deleted successfully.",
    deleteRecordError: "Could not delete: ",
    descriptionUpdateError: "Could not update the description.",
    bioInfo: "Biological Information",
    mythsAndLegends: "Myths & Legends",
    selectSpecies: "Select the Species",
    whichSpecies: "Which of these did you identify?",
    certainty: "Certainty",
    record: "record",
    recordPlural: "records",
    speciesConfirmed: "Species confirmed",
    speciesConfirmedMsg: "The record is ready to upload.",
    speciesConfirmError: "Could not save the selection.",
    syncSuccess: "{{uploaded}} uploaded, {{identified}} identified.",
    syncNoConnection: "Could not sync. Check your internet connection.",
    syncRetryError: "Could not retry synchronization.",
    deletePostTitle: "Delete post",
    deletePostMsg: "Are you sure you want to delete this post?",
    deletePostError: "Could not delete the post.",
    commentPlaceholder: "Write a comment...",
    commentError: "Could not send the comment.",
    attention: "Attention",
  },
  tabs: {
    discover: "Discover",
    observatory: "Observatory",
    scanner: "Scanner",
    records: "Records",
    profile: "Profile",
  },
  common: {
    loading: "Loading...",
    error: "An error occurred",
    save: "Save",
    cancel: "Cancel",
    search: "Search...",
    delete: "Delete",
    all: "All",
    animals: "Animals",
    plants: "Plants",
    back: "Back",
    send: "Send",
    edit: "Edit",
    options: "Options",
    justNow: "Just now",
    minutesAgo: "{{count}}m ago",
    hoursAgo: "{{count}}h ago",
    daysAgo: "{{count}}d ago",
    connectingAI: "Connecting with AI",
    pendingConfirm: "Needs confirmation",
    rejected: "Rejected",
    pendingUpload: "Pending upload",
    close: "Tap to close",
    confirm: "Confirm",
  },
  drafts: {
    noSpeciesDetected: "No species detected",
    noAnimalsOrPlants: "It seems there are no animals or plants here.",
    invalidSpecies: "Invalid species",
    speciesNotInVenezuela: "The detected species does not inhabit Venezuela.",
    rateLimitRetry: "Rate limit — retrying",
    syncTitle: "📡 Record synced",
    syncMessage: "\"{{name}}\" has just been uploaded.",
    identifyingProgress: "Identifying \"{{name}}\"...",
    uploadingProgress: "Uploading \"{{name}}\"...",
    defaultCaptureName: "capture",
    deleteTitle: "Delete pending",
    deleteMessage: "Are you sure you want to delete this pending record? It will not be uploaded.",
    deleteConfirm: "Delete",
  },
  follows: {
    newFollower: "New follower",
    startedFollowing: "started following you.",
    aUser: "A user",
  },
  auth: {
    loginTitle: "Log In",
    registerTitle: "Create Account",
    emailPlaceholder: "Email address",
    passwordPlaceholder: "Password",
    loginButton: "Sign In",
    registerButton: "Sign Up",
    noAccount: "Don't have an account? Sign up",
    hasAccount: "Already have an account? Log in"
  },
  scanner: {
    title: "Scanner",
    takePhoto: "Take Photo",
    chooseGallery: "Gallery",
    identifying: "Identifying...",
    uploading: "Uploading...",
    success: "Captured!",
    loadingPermissions: "Loading permissions...",
    cameraPermission: "Camera permission is required",
    grantPermission: "Grant permission",
    saved: "Saved!",
    savedSubtext: "It will be identified and uploaded in the background",
    capturing: "Capturing...",
    savingLocal: "Saving locally...",
    scanHint: "Point and capture — processed in the background",
    uploadRecord: "Upload record",
    pendingBadge: "{{count}} pending",
    scanCaption: "Capture an image and identify species from the Venezuelan Guayana.",
    pendingCapture: "Pending capture",
    analyzingSpecies: "Analyzing species...",
    retake: "Retake",
    uploadPhoto: "Upload",
    savedToDraft: "Uploaded to draft",
    savedToDraftSubtext: "It will be processed and identified in the background",
    scannerButton: "Identify species",
  },
  createRecord: {
    title: "New record",
    selectPhoto: "Select photo",
    selectCaption: "Tap to choose the sighting image",
    locateOnMap: "Locate on map",
    mapTitle: "Place the sighting",
    chooseLocationInstructions: "Move the pin or change the location before continuing.",
    reviewAndConfirm: "Review and confirm",
    reviewTitle: "Review sighting",
    reviewHint: "Verify the attached photo and chosen location.",
    locationLabel: "Location",
    changeLocationHint: "Tap to choose another location",
    descriptionPlaceholder: "You can add a description...",
    sendToDraft: "Send to draft",
    confirmAndSave: "Confirm and save",
    invalidLocationTitle: "Invalid location",
    invalidLocationMessage: "You must be inside the Venezuelan Guayana.",
  },
  messages: {
    title: "Messages",
    newChat: "New Chat",
    typeMessage: "Type a message...",
    send: "Send",
    noMessages: "No messages",
    chats: "Chats",
    inbox: "Inbox",
    noConversations: "No conversations",
    startChatHint: "Start a chat with someone you follow.",
    startChatButton: "Start chatting",
    chatOptions: "Chat options",
    messageOptions: "Message options",
    closeChat: "Close chat",
    edited: "Edited",
  },
  social: {
    title: "Social",
    discover: "Discover",
    follow: "Follow",
    unfollow: "Unfollow",
    followers: "Followers",
    following: "Following",
    searchUsers: "Search users...",
    discoverUsers: "Discover Users",
    searchByUsername: "Search by username...",
    noUsersFound: "No users found",
    noUsersDiscover: "No users to discover",
    noFollowers: "No followers yet",
    noFollowing: "Not following anyone yet",
    noFollowChat: "You don't follow anyone to chat with yet.",
    startChat: "Start Chat",
  },
  observatory: {
    title: "Observatory",
    liveStream: "Live Stream",
    disconnected: "Stream disconnected",
    disconnectedBadge: "OFFLINE",
    community: "Community",
    publish: "+ Post",
    cancel: "Cancel",
    postTitlePlaceholder: "Post title...",
    postDescPlaceholder: "Write the details here...",
    submitPost: "Publish",
    writeComment: "Write a comment...",
    commentError: "Write a comment to continue.",
    loginToComment: "Log in to comment",
    commentFailed: "Could not send the comment.",
    loginToLike: "Log in to like",
    deletePost: "Delete post?",
    deletePostConfirm: "This action cannot be undone.",
    deleted: "Post deleted",
    deleteFailed: "Could not delete the post.",
    emptyFeed: "No posts yet.",
    emptyFeedHint: "Be the first to share something!",
    refresh: "Refresh",
    followBtn: "Follow",
    followingBtn: "Following",
    sendComment: "Send",
    likeFailed: "Could not update the like.",
    ago: "ago",
    justNow: "Just now",
  },
  records: {
    title: "Records",
    subtitle: "User discoveries",
    searchPlaceholder: "Search species or name...",
    syncing: "Syncing discoveries...",
    empty: "No discoveries here yet.",
  },
  notifications: {
    title: "Notifications",
    empty: "You don't have any notifications yet.",
    newLike: "New like",
    likesCount: "{{count}} likes",
    likedYourPost: "liked",
    peopleLikedPost: "people liked",
    newComment: "New comment",
    commentsCount: "{{count}} comments",
    commentedOnPost: "commented on",
    commentsOnPost: "comments on",
    someone: "Someone",
    yourPost: "your post",
    errorSaveTitle: "Error",
    errorSaveMessage: "Could not save the record.",
  },
  help: {
    title: "Support",
    whatIsEcos: "What is ECOS?",
    whatIsEcosDesc: "ECOS is a mobile app and website designed so that anyone can explore, record, and help protect the nature of the Venezuelan Guayana. It works as a large community where locals, tourists, and experts share photos and data of the plants and animals they find, creating together a great archive to protect this valuable region.",
    screens: "Screens",
    discoverTitle: "Discover",
    discoverDesc: "This is the main screen for exploring. It shows an interactive map of the Guayana with markers for animals and plants that others have found. To help you on your walks, it includes a small panel with weather and forecasts, plus a chat so you can talk and organize with other explorers.",
    scannerTitle: "Scanner",
    scannerDesc: "This is your tool to identify what you see. Using your phone's camera, the app helps you recognize what animal or plant is in front of you and saves the exact spot where you found it. If you're in an area without internet, no worries — you can take the photo and upload it later from your gallery.",
    observatoryTitle: "Observatory",
    observatoryDesc: "This is a direct window into nature. Here you can watch live video streams from hidden cameras at key ecosystem locations. It also has a social media-style wall where the community can write posts, share their adventures, and talk about environmental care.",
    recordsTitle: "Records",
    recordsDesc: "This is the app's public album. In this section you can see an updated list of all photos, discoveries, and identifications shared by all ECOS users, creating a living catalog of what exists in our region.",
    profileTitle: "Profile",
    profileDesc: "This is your personal space within the platform. Here you can see who you follow and who follows you, plus have at hand the history of all the species you've discovered and messages you've posted in the community.",
    mission: "What does ECOS aim to achieve?",
    missionDesc: "Above all, the main goal is to connect people with nature through knowledge. The platform is based on a fundamental idea: when someone truly understands what they are seeing, they value and care for it much more. From that connection and making this information accessible, ECOS seeks to show the world the incredible richness of the Venezuelan Guayana, giving anyone an easy-to-use tool so we can all help monitor, document, and conserve our environment.",
  },
  discover: {
    speciesDefault: "Species",
    dangerLevel: "Danger Level",
    feeding: "Feeding",
    sightingNotes: "Sighting Notes",
    location: "Location",
    registrationDate: "Registration Date",
    habitat: "Habitat",
    riskLevel: "Risk Level",
    bioDescription: "Biological Description",
    curiosities: "Curiosities",
    mythsAndLegends: "Myths & Legends",
    noImage: "No image",
    notAvailable: "N/A",
  },
};
